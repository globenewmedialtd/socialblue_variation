 ...
 */
function socialblue_variation_form_system_theme_settings_alter(&$form, FormStateInterface &$form_state, $form_id = NULL) {
  ...
  // If the default theme is either socialblue_variation or socialsaas then extend
  // the form in the appearance section.
  if (array_key_exists('socialblue', \Drupal::service('theme.manager')->getActiveTheme()->getBaseThemes())) {
    if ($active_theme == $system_theme_settings) {
      ...
      $form['#submit'][] = 'socialblue_variation_save_email_logo';
      ...
      if (\Drupal::configFactory()->get('system.theme')->get('admin') === 'gin') {
        $form['#submit'][] = 'socialblue_variation_update_gin_color_settings';
      }
    }
...
/**
 * Updates the gin color settings from socialblue_variation it's config.
 *
 ...
 */
function socialblue_variation_update_gin_color_settings(array $form, FormStateInterface $form_state) {
  // Grab the default socialblue_variation colors, these are set if the color settings
  // aren't overridden yet.
  $default_colors = \Drupal::configFactory()->getEditable('socialblue_variation.settings')->getRawData();
  ...
  $socialblue_variation_colors = \Drupal::configFactory()->getEditable('color.theme.socialblue_variation')->getRawData();
  ...
  $brand_primary = !empty($socialblue_variation_colors) ? $socialblue_variation_colors['palette']['brand-primary'] : $default_colors['color_primary'];
  $brand_secondary = !empty($socialblue_variation_colors) ? $socialblue_variation_colors['palette']['brand-secondary'] : $default_colors['color_secondary'];
 ...
 * @throws \Drupal\Core\Entity\EntityStorageException
 */
function socialblue_variation_save_email_logo(array $form, FormStateInterface $form_state) {
